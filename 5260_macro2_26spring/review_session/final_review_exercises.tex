...
\begin{enumerate}[label=\arabic*.]
    ...
    \end{equation*}
    where $0<\beta<1$, $\kappa>0$, and $e_t$ is a cost-push shock. The IS curve is given by
    \begin{align*}
        x_t = \mathbb{E}_t x_{t+1} - \frac{1}{\sigma} (\hat{\iota}_t - \mathbb{E}_t \pi_{t+1}) + \mu_t,
    \end{align*}
    ...
    \begin{enumerate}[label=(\alph*)]
        \item (8 points) Derive the second-order expectational difference equation for $x_t$.
        
        \item (10 points) Guess that the solution takes the form
        \begin{equation*}
            x_t = a x_{t-1} + b e_t + c e_{t-1}.
        \end{equation*}
        Use coefficient matching to solve for $a$, $b$, and $c$.
        
        \item (4 points) Derive the implied solution for inflation $\pi_t$.
    \end{enumerate}
    ...
    \begin{align*}
        \hat{\mathbb{E}}_{t-1}\bar{\pi}_T=\bar{\pi}_t
        \qquad
        \text{for all } T\geq t.
    \end{align*}

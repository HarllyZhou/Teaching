...
\begin{enumerate}[label=(\arabic*)]
    \item (4 points) Derive the price setting equation. (Hint: The marginal product of labour is $\frac{\partial Y}{\partial N} = \frac{\mathcal{A}}{2\sqrt{N}}$.)
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*),resume]
    \item (6 points) In the same diagram, show how the domestic central bank should respond in order to keep the exchange rate fixed. Denote the new equilibrium by point $B$. If you shift any curve, label the new curve with subscript $B$.
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*),resume]
    \item (4 points) Calculate the domestic nominal interest rate that the central bank should target to maintain the fixed exchange rate regime. Can this nominal interest rate be targeted?
...
\textbf{Question}: Show that the aggregate output $Y$ can be expressed as

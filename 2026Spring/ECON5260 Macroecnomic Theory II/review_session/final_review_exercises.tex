...
\begin{enumerate}[label=\arabic*.]
    ...
    \textbf{Part II. Imperfect Knowledge and Inflation Anchoring}
...
\begin{enumerate}[label=(\alph*), resume]
    ...
    \item (6 points)
    Using the forecasting model above, show that
    \begin{align*}
        \hat{\mathbb{E}}_t s_T
        =
        \lambda_x^{-1}
        \left[
        \rho^{T-t+1}(1-\bar{\omega})\varphi_{t-1}
        -
        (1-\gamma)\bar{\pi}_t
        \right].
    \end{align*}
    ...
    \item (5 points)
    Starting from the aggregate supply curve in part (b), use the expression for $\hat{\mathbb{E}}_t s_T$ derived in part (e) to show that the true inflation-generating process can be written as
    \begin{align*}
        \pi_t
        =
        \gamma\pi_{t-1}
